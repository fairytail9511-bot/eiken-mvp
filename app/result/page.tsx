...
function pickSpeechAIFeedback(payload: any): SpeechAIFeedback | null {
  const cands = [payload, payload?.feedback, payload?.data, payload?.result, payload?.output, payload?.speechFeedback];
      ...
      >
        ...
        <div style={{ flex: "none" }}>
          <div style={{ color: "#fff" }}>
            ...
            {elapsed ? (
              <div style={{ fontSize: 12, opacity: 0.9, marginTop: 2 }}>（所要時間：{elapsed}）</div>
            ) : null}
          </div>
          ...
          {(error || regenError) && (
            <div style={{ marginTop: 10 }}>
              ...
              {regenError && (
                <div style={{ color: "#fecaca", fontSize: 13, whiteSpace: "pre-wrap" }}>{regenError}</div>
              )}
            </div>
          )}
        </div>
